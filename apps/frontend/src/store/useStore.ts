...
export const useStore = create<AppState>()(
  persist(
    (set) => ({
      theme: 'auto',
      tiles: [
        { id: 'tile-1', type: 'sample', x: 0, y: 0, w: 10, h: 2, hidden: false },
        { id: 'tile-2', type: 'sample', x: 10, y: 0, w: 10, h: 2, hidden: false },
      ],
      notes: [],
      editMode: false,
      gridColumns: (() => {
      ...
      addTile: (type) =>
        set((s) => {
          const newH = ['server', 'rocketmeals', 'weather', 'news', 'route', 'tasks', 'notes', 'aiagent', 'dockerlogs', 'speechtotext', 'reactcoderender', 'speechlibrary', 'apiswitch'].includes(type) ? 4 : type === 'postit' ? 3 : 2
