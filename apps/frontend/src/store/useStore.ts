...
export const useStore = create<AppState>()(
  persist(
    (set) => ({
      ...
      addTile: (type) =>
        set((s) => {
          const newH = ['server', 'rocketmeals', 'weather', 'news', 'route', 'tasks', 'notes', 'aiagent', 'dockerlogs', 'speechtotext', 'reactcoderender', 'speechlibrary'].includes(type) ? 4 : type === 'postit' ? 3 : 2
